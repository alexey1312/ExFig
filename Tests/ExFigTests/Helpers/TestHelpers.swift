...
extension VariablesMeta {
    ...
    static func make(
        ...
        let variableIds = variables.map { "\"VariableID:\($0.id)\"" }.joined(separator: ", ")

        let variablesJson = variables.map { variable in
            let valuesJson = variable.valuesByMode.map { modeId, color in
                "\"\(modeId)\": {\"r\": \(color.r), \"g\": \(color.g), \"b\": \(color.b), \"a\": \(color.a)}"
            }.joined(separator: ", ")
