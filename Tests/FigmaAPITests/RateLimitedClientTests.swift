...
final class RateLimitedClientAsyncTests: XCTestCase {
    func testRetriesOn429WithRetryAfter() async throws {
        let rateLimiter = SharedRateLimiter(requestsPerMinute: 600.0, burstCapacity: 10.0)
        let baseClient = SequencedClient(
            responses: [
                .failure(HTTPError(statusCode: 429, retryAfter: 0.01, body: Data())),
                .success(Data("ok".utf8)),
            ]
        )
        let retryPolicy = RetryPolicy(maxRetries: 1, baseDelay: 0.001, maxDelay: 0.01, jitterFactor: 0.0)
        ...
        let result = try await client.request(DummyEndpoint())

        XCTAssertEqual(result, "ok")
        XCTAssertEqual(baseClient.callCount, 2)
    }
}
...
private struct DummyEndpoint: Endpoint {
    ...
    func content(from response: URLResponse?, with body: Data) throws -> String {
        _ = response
        return String(data: body, encoding: .utf8) ?? ""
    }
}

private final class SequencedClient: Client, @unchecked Sendable {
    ...
    func request<T: Endpoint>(_ endpoint: T) async throws -> T.Content {
        let result: Result<Data, Error> = {
            ...
            guard !responses.isEmpty else {
                return .failure(HTTPError(statusCode: 500, retryAfter: nil, body: Data()))
            }
            return responses.removeFirst()
        }()
        switch result {
        case let .success(data):
            return try endpoint.content(from: nil, with: data)
