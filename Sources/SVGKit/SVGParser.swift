...
public struct SVGColor: Equatable, Sendable {
    ...
    private static let rgbRegex: NSRegularExpression = {
        ...
        try! NSRegularExpression(pattern: #"rgba?\s*\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*(?:,\s*([\d.]+))?\s*\)"#, options: [])
    }()
...
public final class SVGParser: @unchecked Sendable { // swiftlint:disable:this type_body_length
    ...
    private static let urlReferenceRegex: NSRegularExpression = {
        // swiftlint:disable:next force_try
        try! NSRegularExpression(pattern: #"url\(#([^)]+)\)"#, options: [])
    }()
    ...
    private func parseCSSRules(_ css: String) {
        ...
        let matches = Self.cssRegex.matches(in: css, options: [], range: NSRange(css.startIndex..., in: css))
    ...
    private func resolveFill(_ fillValue: String?) -> SVGFill {
        ...
        if fill.hasPrefix("url(#") {
            if let match = Self.urlReferenceRegex.firstMatch(in: fill, options: [], range: NSRange(fill.startIndex..., in: fill)),
