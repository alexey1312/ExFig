...
final class ImagesLoader: ImageLoaderBase, @unchecked Sendable {
    ...
    func load(filter: String? = nil) async throws -> ImagesLoaderOutput {
    ...
    private func loadFromSingleFile(filter: String? = nil) async throws -> ImagesLoaderOutput {
    ...
    private func loadFromLightAndDarkFile(filter: String? = nil) async throws -> ImagesLoaderOutput {
        ...
        switch (platform, params.android?.images?.format) {
        case (.android, .png), (.android, .webp), (.ios, _):
            return try await loadRasterImagesFromMultipleFiles(filesToLoad: filesToLoad, filter: filter)
        default:
            return try await loadVectorImagesFromMultipleFiles(filesToLoad: filesToLoad, filter: filter)
        }
    }
    ...
    private func loadRasterImagesFromMultipleFiles(
        ...
        let results = try await withThrowingTaskGroup(
            ...
            for (key, fileId) in filesToLoad {
                group.addTask { [key, fileId, filter, scales] in
